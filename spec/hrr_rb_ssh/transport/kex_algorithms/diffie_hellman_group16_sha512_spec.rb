RSpec.describe HrrRbSsh::Transport::KexAlgorithms::DiffieHellmanGroup16Sha512 do
  let(:name){ 'diffie-hellman-group16-sha512' }
  ...
  let(:remote_dh){
    dh = HrrRbSsh::OpenSSLCompat.new_dh_pkey(
      p: OpenSSL::BN.new(dh_group16_p, 16),
      g: OpenSSL::BN.new(dh_group16_g)
    )
    dh
  }
  ...
  describe '::P' do
    it "has diffie hellman group 16's p" do
      expect(described_class::P).to eq dh_group16_p
    end
  end

  describe '::G' do
    it "has diffie hellman group 16's g" do
      expect(described_class::G).to eq dh_group16_g
    end
  end
