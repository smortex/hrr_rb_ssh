RSpec.describe HrrRbSsh::Transport::KexAlgorithms::DiffieHellmanGroup15Sha512 do
  let(:name){ 'diffie-hellman-group15-sha512' }
  ...
  let(:remote_dh){
    dh = HrrRbSsh::OpenSSLCompat.new_dh_pkey(
      p: OpenSSL::BN.new(dh_group15_p, 16),
      g: OpenSSL::BN.new(dh_group15_g)
    )
    dh
  }
  ...
  describe '::P' do
    it "has diffie hellman group 15's p" do
      expect(described_class::P).to eq dh_group15_p
    end
  end

  describe '::G' do
    it "has diffie hellman group 15's g" do
      expect(described_class::G).to eq dh_group15_g
    end
  end
